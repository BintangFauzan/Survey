...
export default function PublicSurveyPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        // Ambil data survei dan prodi secara paralel
        const [surveyRes, prodiRes] = await Promise.all([
          apiClient.get(`/survey/${slug}`),
          apiClient.get("/prodi")
        ]);
        
        if (surveyRes.data && surveyRes.data.survey) {
          setSurvey(surveyRes.data.survey);
        }

        if (prodiRes.data && prodiRes.data.data) {
          // Backend sekarang menggunakan get() bukan paginate(15)
  ...
  const handleSubmit = async (e) => {
    ...
    const totalQuestions = survey.kategori?.pertanyaan?.length || 0;
    ...
    if (!namaResponden || !jabatan || !nomorWhatsapp || !prodiId) {
      alert("Harap lengkapi data diri Anda (Nama, Jabatan, Prodi, dan No. WhatsApp) sebelum mengirim.");
      setSubmitting(false);
      return;
    }
  ...
  const pertanyaan = survey.kategori?.pertanyaan || [];
