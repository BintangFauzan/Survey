...
export default function DefaultInputs({
  ...
  const [prodiId, setProdiId] = useState(userProdiId || "");
  ...
  useEffect(() => {
    ...
    if (isEdit && dataToEdit) {
      ...
      setProdiId(dataToEdit.prodi_id || "");
    }
  }, [dataToEdit, isEdit]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (isEdit) {
        const payload = { title, description, status, prodi_id: prodiId };
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: `Survey berhasil ${isEdit ? "diperbarui" : "dibuat"}`,
        timer: 1500,
      });
    } catch (err) {
      const errorMessage =
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          readOnly
          disabled
          className="mt-1 block w-full rounded-md p-3 border-gray-300 bg-gray-100 shadow-sm sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400 cursor-not-allowed"
        />
      </div>
