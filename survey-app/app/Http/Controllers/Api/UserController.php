    ...
    public function register(Request $request){
       try{
         $request->validate([
            ...
            'email' => 'required',
            'password' => 'required',
            'prodi_id' => 'nullable|exists:prodi,id' //wajib untuk role admin
        ]);

        // aturan khusus admin
        $prodi_id = $request->prodi_id;
        if ($request->role === 'admin' && !$prodi_id) {
            // wajib terikat prodi
            return response()->json(['message' => 'Admin harus terikat dengan prodi.'], 422);
        }
        // aturan super admin
